...
<div class="bjui-pageContent">
    <form action="/index.php/Home/Dep/edit/navTabId/<?php echo CONTROLLER_NAME;?>" class="pageForm" data-toggle="validate">
		<input type="hidden" name="id" value="<?php echo ($id); ?>"><input type="hidden" name="type" value="<?php echo ($type); ?>">
        <div class="pageFormContent" data-layout-h="0">
          <table class="table table-condensed table-hover" width="100%">
           <tbody>
			<tr><td>
                <label for="j_name" class="control-label x85">所属部门：</label><select  name="pid" data-toggle="selectpicker" data-rule="required">
	          ...
	          <?php switch($v["level"]): case "0": ?>+<?php break;?> <?php case "1": ?>+--<?php break; case "2": ?>+------<?php break; default: endswitch;?>
	            <?php echo ($v["title"]); ?> <?php if($v["status"] == 1 ): else: ?><font color=red>[未启用]</font><?php endif; endforeach; endif; ?>
             </select>
            </td></tr>
            <tr><td>
                <label for="j_title" class="control-label x85"><?php if($type == 0 ): ?>部门<?php else: ?>职位<?php endif; ?>名称：</label>
